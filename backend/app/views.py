...
class question_type_listView(generics.ListAPIView):
    ...
    def get_queryset(self):
        if self.request.method == 'GET':
            return question_type.objects.filter(deleted=0)
        return question_type.objects.filter(deleted=0)
...
class CollegeListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        return college_master.objects.filter(deleted=0)
...
def get_skills(request):
    skills = skills_master.objects.filter(deleted=0) # Accessing the skill model directly
...
def get_tests_candidates_answer(request):
    tests_candidates_ans=tests_candidates_answers.objects.filter(deleted=0)
...
def get_course_schedule(request):
    cs_set = course_schedule.objects.select_related('student_id', 'course_id', 'trainer_id').filter(deleted=0)
...
def get_course_content_feedback(request):
    course_set = course_content_feedback.objects.select_related('course_id', 'student_id', 'topic_id', 'trainer_id').filter(deleted=0)
...
def get_announcement_master(request):
    announce_set = announcement_master.objects.select_related('college_id', 'trainer_id').filter(deleted=0)
...
class ExcelImportView_Questions(APIView):
    def post(self, request, format=None):
        ...
        for index, row in df.iterrows():
            ...
            skill_type_id = skills_master.objects.filter(skill_name=skills_name).first().id
            topic_id = topic_master.objects.filter(topic=topic_name).first().id

            df.at[index, 'question_type_id'] = question_type_id
            df.at[index, 'skill_type_id'] = skill_type_id
            df.at[index, 'topic_id'] = topic_id
